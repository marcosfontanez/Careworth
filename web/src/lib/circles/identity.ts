/** Circle identity helpers — mirrors native `lib/circleIdentity.ts` for web. */
...
const CONFESSION_SLUGS = new Set(["confessions", "shift-confessions"]);
const CAREGIVER_CATEGORIES = new Set(["caregivers", "family", "caregiver"]);

export function isConfessionSlug(slug: string | null | undefined): boolean {
  if (!slug) return false;
  const key = slug.trim().toLowerCase();
  return key === "shift-confessions" || CONFESSION_SLUGS.has(key);
}
...
export function resolveWelcomeCopy(slug: string | undefined, metadata?: CircleIdentityMetadata): string {
  const custom = metadata?.welcomeCopy?.trim();
  if (custom) return custom;
  return isConfessionSlug(slug) ? CONFESSIONS_WELCOME_COPY : DEFAULT_WELCOME_COPY;
}
...
export function resolveCircleRules(
  ...
  if (isConfessionSlug(slug)) return CONFESSIONS_RULES.slice(0, 5);

  const rules = [...BASE_RULES];
  const showMedical =
    PUBLIC_HEALTH_SLUGS.has((slug ?? "").trim().toLowerCase()) ||
