...
COMMENT ON COLUMN public.profiles.audience_role IS
  'Self-selected onboarding audience (healthcare_worker, caregiver_family, here_to_learn, …). Not a verified clinical credential.';

COMMENT ON COLUMN public.profiles.onboarding_completed_at IS
  'When the user finished (or skipped) post-signup onboarding. NULL → show onboarding after legal ack.';
...
COMMENT ON COLUMN public.profiles.creator_audience_tags IS
  'Optional creator tags: who this creator primarily makes content for (healthcare_workers, students, …).';
...
COMMENT ON COLUMN public.communities.onboarding_topics IS
  'Topic tokens for onboarding Circle suggestions (humor, caregiver_support, career_exploration, …).';
