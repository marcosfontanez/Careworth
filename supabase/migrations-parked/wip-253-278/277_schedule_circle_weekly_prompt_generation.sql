...
--
-- Timezone: UTC-fixed. 10:00 UTC ≈ 06:00 America/New_York during EDT.
--
-- PREREQUISITE — create these two Vault secrets ONCE (Dashboard → Project
-- Settings → Vault, or SQL), then this job activates on the next deploy/run:
--
...
begin
  ...
  if not v_has_cron or not v_has_net then
    raise notice '[277] pg_cron/pg_net not available — generation schedule skipped. Schedule generate-circle-weekly-prompts via the Dashboard instead.';
    return;
  end if;
  ...
  if not v_has_url or not v_has_secret then
    raise notice '[277] Vault secrets project_url / circle_prompts_cron_secret missing — generation schedule skipped. Create them (see header) and re-run this migration.';
    return;
  end if;
