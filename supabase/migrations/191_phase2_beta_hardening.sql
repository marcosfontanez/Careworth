...
-- 1) Validate creator gift context_id server-side
-- 2) Enable Supabase Realtime on public.notifications

-- ---------------------------------------------------------------------------
-- 1. economy_send_creator_gift — strict context validation
-- ---------------------------------------------------------------------------
...
comment on function public.economy_send_creator_gift(uuid, uuid, text, uuid, text) is
  'Atomic creator gift: validates context exists and matches recipient; debits Sparks and credits Diamonds.';

-- ---------------------------------------------------------------------------
-- 2. Realtime — public.notifications (INSERT/UPDATE for in-app badge refresh)
-- ---------------------------------------------------------------------------
do $$
begin
  if not exists (
