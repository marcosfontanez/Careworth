-- Allow staff to grant shop borders (including IAP-listed borders) to any recipient.
-- Same RPC remains admin-gated via _economy_is_admin(); audit rows remain in shop_admin_item_grants.
...
comment on function public.economy_admin_grant_shop_item(uuid, uuid, text, text) is
  ...
  'for any recipient (including IAP-listed shop borders). Audit via shop_admin_item_grants.';
