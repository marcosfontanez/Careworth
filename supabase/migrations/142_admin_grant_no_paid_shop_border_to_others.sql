-- =============================================================================
-- 142: Paid Pulse Shop borders (IAP) — admin grant only to self (QA), not to other users.
-- Regular users must buy (self) or receive an IAP gift through normal checkout.
-- Free-in-shop borders (metadata.free_in_shop) and non–shop-item borders unchanged.
-- =============================================================================
...
begin
  ...
  if v_it.type = 'spark_pack' then
    ...
    if v_paid_shop_border and p_recipient_user_id is distinct from v_admin then
      raise exception 'admin_grant_purchase_only_border'
        using message = 'This border is purchase-only. Grant it only to your own account for QA, or users must buy or receive a store gift.';
    end if;
