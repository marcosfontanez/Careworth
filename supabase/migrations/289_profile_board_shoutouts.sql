-- ============================================================
-- Pulse Board shoutouts, rate limits, pin + notify
-- Reconciled from WIP migrations-parked/wip-253-278 (Phase 3)
-- ============================================================

-- ---------- source: 259_profile_board_shoutouts.sql ----------
-- Migration 259 Â· Pulse Board shoutouts (visitor public wall on My Pulse)
--
...
-- ---------------------------------------------------------------------------
-- 4. RLS â€” reads only; mutations via RPC
-- ---------------------------------------------------------------------------
...
-- ---------- source: 260_profile_board_shoutout_rate_limit.sql ----------
-- Migration 260 Â· Pulse Board shoutout rate limits (V1 hardening)
--
...
-- ---------- source: 261_profile_board_pin_and_notify.sql ----------
-- Migration 261 Â· Pulse Board pin + owner notification (Phase 2B)
--
-- 1. Owner pin/unpin (single active pin per board via pinned_at)
-- 2. Notify profile owner when a visitor leaves a shoutout

-- ---------------------------------------------------------------------------
-- 1. post_profile_board_shoutout â€” owner notification after successful insert
-- ---------------------------------------------------------------------------
create or replace function public.post_profile_board_shoutout(
  p_profile_owner_id uuid,
  p_body text
...
-- ---------------------------------------------------------------------------
-- 2. moderate_profile_board_shoutout â€” pin / unpin (owner/staff)
-- ---------------------------------------------------------------------------
create or replace function public.moderate_profile_board_shoutout(
  p_shoutout_id uuid,
  p_action text
